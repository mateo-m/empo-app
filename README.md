...
Drag any of these onto the Empo icon, share them from another app, or use the Files picker from the library's import button. Empo identifies the engine and its version, picks the correct game core and Ruby interpreter, and extracts artwork from `Game.exe` if present. Empo writes everything to its sandbox, so the original imported folder stays unchanged.
