...
enum ControlsZone {
    ...
    /// header strip plus the taller of the d-pad and the 2x2 button
    /// cluster, with the zone paddings. The screen drag blocks at
    /// this limit unless the profile opts into overlay mode.
    static let requiredEditZoneHeight: CGFloat = 320

    /// Vertical space the edit header (banner pill + action row)
    /// reserves at the top of the controls zone. The controls clamp
    /// by CENTER, so the strip includes headroom for half a large
    /// button above its center — otherwise a wide control still
    /// pokes over the action row.
    static let editHeaderStrip: CGFloat = 96
    ...
    static func toolbarOrigin(
        safeArea: EdgeInsets, geoSize: CGSize, btnSize: CGFloat, gap: CGFloat, count: CGFloat
    ) -> CGPoint {
    ...
    static func absolutePosition(
        for relativeCenter: CGPoint, in size: CGSize, controlSize: CGSize, safeArea: EdgeInsets,
        ...
        return clampToRoundedCorners(
            CGPoint(x: cx, y: cy), controlHalf: max(hw, hh), zone: zone, radii: radii)
    }

    static func clampToSafeArea(
        _ point: CGPoint, controlSize: CGFloat, geoSize: CGSize, safeArea: EdgeInsets, controlsMinY: CGFloat
    ) -> CGPoint {
        let pad = padding + innerPadding
    ...
    static func clampToRoundedCorners(
        _ point: CGPoint, controlHalf: CGFloat, zone: CGRect, radii: (top: CGFloat, bottom: CGFloat)
    ) -> CGPoint {
        var p = point
        ...
        for corner in corners {
            let inCornerX =
                (p.x < corner.cx && corner.cx <= zone.minX + max(radii.top, radii.bottom))
                || (p.x > corner.cx && corner.cx >= zone.maxX - max(radii.top, radii.bottom))
            let inCornerY =
                (p.y < corner.cy && corner.cy <= zone.minY + max(radii.top, radii.bottom))
